...
class TestRewrite(unittest.TestCase):
    ...
    def rewrite_helper(self, constraints, q_str):
        rewriter = Rewriter()
    ...
    def test_simple_enumerate(self):
        ...
        self.rewrite_helper(constraints, q_before_str)


    def test_redmine_enumerate(self):
...
class TestEnumeration(unittest.TestCase):
    ...
    def test_add_limit_one_rewrite(self):
        ...
        q = RewriteQuery(q_str, parse(q_str))
        rewritten_queries = rewriter.rewrite(constraints, q)
        self.assertEqual(len(rewritten_queries), 0)

        q_str = 'select * from R where a = 1 and b = 1'
        q = RewriteQuery(q_str, parse(q_str))
        rewritten_queries = rewriter.rewrite(constraints, q)
        self.assertEqual(len(rewritten_queries), 1)
    ...
    def test_rewrite_types(self):
        ...
        q = RewriteQuery(q_str, parse(q_str))
        rewritten_queries = rewriter.rewrite(constraints, q)
        self.assertEqual(len(rewritten_queries), 3)
        self.assertTrue(rewritten_queries[0].rewrites == [rule.RewriteNullPredicate(c1)] or
            rewritten_queries[0].rewrites == [rule.RewriteNullPredicate(c2)])
        self.assertTrue(rewritten_queries[1].rewrites == [rule.RewriteNullPredicate(c2)] or
            rewritten_queries[1].rewrites == [rule.RewriteNullPredicate(c1)])
        self.assertTrue(rewritten_queries[2].rewrites == [rule.RewriteNullPredicate(c1), rule.RewriteNullPredicate(c2)] or
