...
from mo_parsing import ParseSyntaxException
...
from utils import exp_recorder, generate_query_param_single,\
    generate_query_params, get_field_value, get_str_hash, test_query_result_equivalence,\
    load_json_queries
...
class EvalQuery:
    def __init__(self, rewrite_q, connect_str) -> None:
        ...
        self.raw = generate_query_param_single(rewrite_q.q_raw, connect_str, {})
...
def evaluate_queries(qs, stage, connect_str):
    ...
    for q in tqdm(qs):
        try:
            if stage == "before":
                q.before = Evaluator.evaluate_query("Explain " + q.raw, connect_str)
                for i in range(repeat):
                    q.before_time += Evaluator.evaluate_actual_time(q.raw, connect_str)
                q.before_time /= repeat
            elif stage == "after":
                q.after = Evaluator.evaluate_query("Explain " + q.raw, connect_str)
                for i in range(repeat):
                    q.after_time += Evaluator.evaluate_actual_time(q.raw, connect_str)
                q.after_time /= repeat
        except:
...
if __name__ == "__main__":
    ...
    cnt = 0
    speedups = []
    outfile = "log/db/%s_db_speedup" % appname
