...
-- Original Query
SELECT 1 AS "one" FROM versions WHERE LOWER(versions.name) = 'rzolibgympsnfxgeleiuvbmlsohechkpiemqxpdrdhblcpthmmxikwqpjnpiifsplucmptxktiddpzinemjtevarclsjqvgynckrlzxxfomnkonslzkpxjijjdhtpvlaipu' AND versions.id <> 7541 AND versions.project_id IS NULL LIMIT 8;
-- Rewritten Queries
SELECT 1 AS "one" FROM versions WHERE LOWER(versions.name) = 'rzolibgympsnfxgeleiuvbmlsohechkpiemqxpdrdhblcpthmmxikwqpjnpiifsplucmptxktiddpzinemjtevarclsjqvgynckrlzxxfomnkonslzkpxjijjdhtpvlaipu' AND versions.id <> 7541 AND False LIMIT 8;
