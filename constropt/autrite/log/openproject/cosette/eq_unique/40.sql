...
-- Original Query
SELECT projects.* FROM projects INNER JOIN (SELECT projects.id, versions.id AS version_id, projects.lft, projects.rgt, versions.sharing FROM projects INNER JOIN versions ON projects.id = versions.project_id WHERE versions.id = 13) AS sharing ON True;
-- Rewritten Queries
