...
-- Original Query
SELECT COUNT(DISTINCT users.id) FROM users LEFT OUTER JOIN user_preferences ON user_preferences.user_id = users.id WHERE users.type IN ('User', 'User', 'User', 'User') AND users.type = 'Group';
-- Rewritten Queries
SELECT COUNT(DISTINCT users.id) FROM users INNER JOIN user_preferences ON user_preferences.user_id = users.id WHERE users.type IN ('User', 'User', 'User', 'User') AND users.type = 'Group';
