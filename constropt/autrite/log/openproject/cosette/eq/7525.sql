...
-- Rewritten Queries
SELECT relations.* FROM relations WHERE relations.hierarchy = 0 AND hierarchy + relates + duplicates + follows + blocks + includes + requires = 1 AND (relations.blocks IS NULL OR relations.blocks NOT IN ('1')) ORDER BY relations.id DESC LIMIT 3 OFFSET 1;
SELECT relations.* FROM relations WHERE relations.hierarchy = 0 AND hierarchy + relates + duplicates + follows + blocks + includes + requires = 1 AND (False OR relations.blocks NOT IN ('1')) ORDER BY relations.id DESC LIMIT 3 OFFSET 1;
