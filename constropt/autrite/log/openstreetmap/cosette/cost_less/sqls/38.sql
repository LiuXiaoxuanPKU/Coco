...
-- Original Query
SELECT note_comments.* FROM note_comments LEFT OUTER JOIN users ON users.id = note_comments.author_id WHERE note_comments.note_id = 6753 AND (users.status IN ('pending', 'pending') OR users.status IS NULL) ORDER BY note_comments.created_at ASC LIMIT 6;
-- Rewritten Queries
SELECT note_comments.* FROM note_comments LEFT OUTER JOIN users ON users.id = note_comments.author_id WHERE note_comments.note_id = 6753 AND users.status IS NULL ORDER BY note_comments.created_at ASC LIMIT 6;
