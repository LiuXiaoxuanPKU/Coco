...
-- Original Query
SELECT users.* FROM users INNER JOIN friends ON users.id = friends.friend_user_id INNER JOIN users AS befriendees_friends ON befriendees_friends.id = friends.friend_user_id WHERE friends.user_id = 223 AND users.status IN ('pending', 'pending');
-- Rewritten Queries
SELECT users.* FROM users INNER JOIN friends ON users.id = friends.friend_user_id WHERE friends.user_id = 223 AND users.status IN ('pending', 'pending');
