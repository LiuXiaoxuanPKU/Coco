...
-- Original Query
SELECT 1 AS "one" FROM users INNER JOIN friends ON users.id = friends.friend_user_id INNER JOIN users AS befriendees_friends ON befriendees_friends.id = friends.friend_user_id WHERE friends.user_id = 1665 AND users.status IN ('pending', 'pending') LIMIT 2;
-- Rewritten Queries
SELECT 1 AS "one" FROM users INNER JOIN friends ON users.id = friends.friend_user_id WHERE friends.user_id = 1665 AND users.status IN ('pending', 'pending') LIMIT 2;
