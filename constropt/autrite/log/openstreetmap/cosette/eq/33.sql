...
-- Original Query
SELECT 1 AS "one" FROM friends INNER JOIN users ON users.id = friends.friend_user_id WHERE friends.user_id = 513 AND users.status IN ('pending', 'pending') AND friends.friend_user_id = 2862 LIMIT 1;
-- Rewritten Queries
SELECT 1 AS "one" FROM friends WHERE friends.user_id = 513 AND friends.friend_user_id = 2862 LIMIT 1;
