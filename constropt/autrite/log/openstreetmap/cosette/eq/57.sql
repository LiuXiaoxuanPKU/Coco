...
-- Original Query
SELECT DISTINCT timestamp AS alias_0, gpx_files.id FROM gpx_files INNER JOIN users ON users.id = gpx_files.user_id INNER JOIN gpx_file_tags ON gpx_file_tags.gpx_id = gpx_files.id WHERE gpx_files.visibility IN ('public', 'public') AND gpx_files.visible = False AND users.display_name = 'wsxutrbndpfzuyzmgjrwjvq' AND gpx_file_tags.tag = 'hsnacuupfcumxlxbajejkbmxnobuwfijuimxeudnzfxlkaeacghreinwcfvaqhlutmcdumiqnrwzofsymugyjrhlovqxstcutmogmjmfyfdrmqspteplqzfxqnomxhsdao' ORDER BY timestamp DESC LIMIT 7;
-- Rewritten Queries
