...
-- Rewritten Queries
SELECT 1 AS "one" FROM diary_entries INNER JOIN users ON users.id = diary_entries.user_id WHERE users.status IN ('pending', 'pending') AND diary_entries.visible = False LIMIT 1 OFFSET 1;
