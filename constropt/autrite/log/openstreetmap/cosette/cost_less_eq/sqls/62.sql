...
-- Original Query
SELECT 1 AS "one" FROM diary_entries INNER JOIN users ON users.id = diary_entries.user_id LEFT OUTER JOIN languages ON languages.code = diary_entries.language_code WHERE users.status IN ('pending', 'pending') AND diary_entries.language_code = 'icyirxgefnuhgpytcomrgejtfjm' AND diary_entries.visible = True LIMIT 1 OFFSET 1;
-- Rewritten Queries
SELECT 1 AS "one" FROM diary_entries INNER JOIN users ON users.id = diary_entries.user_id WHERE users.status IN ('pending', 'pending') AND diary_entries.language_code = 'icyirxgefnuhgpytcomrgejtfjm' AND diary_entries.visible = True LIMIT 1 OFFSET 1;
