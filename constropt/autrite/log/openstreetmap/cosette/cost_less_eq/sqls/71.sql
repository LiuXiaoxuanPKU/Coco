...
SELECT notes.* FROM notes INNER JOIN note_comments ON note_comments.note_id = notes.id AND note_comments.visible = True WHERE (notes.status = 'open' OR notes.status = 'open' AND notes.closed_at > '2022-08-22 21:22:00.499269') AND note_comments.author_id = 5544 ORDER BY updated_at DESC LIMIT 5;
SELECT DISTINCT notes.* FROM notes INNER JOIN note_comments ON note_comments.note_id = notes.id AND note_comments.visible = True WHERE (notes.status = 'open' OR notes.status = 'open' AND notes.closed_at > '2022-08-22 21:22:00.499269') AND note_comments.author_id = 5544 ORDER BY updated_at DESC LIMIT 5;
