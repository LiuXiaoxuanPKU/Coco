...
-- Original Query
SELECT note_comments.* FROM note_comments LEFT OUTER JOIN users ON users.id = note_comments.author_id WHERE note_comments.note_id = 6104 AND (users.status IN ('pending', 'pending') OR users.status IS NULL) ORDER BY note_comments.created_at ASC;
-- Rewritten Queries
SELECT note_comments.* FROM note_comments LEFT OUTER JOIN users ON users.id = note_comments.author_id WHERE note_comments.note_id = 6104 AND users.status IS NULL ORDER BY note_comments.created_at ASC;
