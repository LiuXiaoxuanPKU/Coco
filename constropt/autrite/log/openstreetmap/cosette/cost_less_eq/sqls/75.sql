...
SELECT notes.* FROM notes INNER JOIN note_comments ON note_comments.note_id = notes.id AND note_comments.visible = False WHERE (notes.status = 'open' OR notes.status = 'open' AND notes.closed_at > '2022-08-22 21:21:59.933822') AND note_comments.author_id = 4062 ORDER BY updated_at DESC LIMIT 2;
SELECT DISTINCT notes.* FROM notes INNER JOIN note_comments ON note_comments.note_id = notes.id AND note_comments.visible = False WHERE (notes.status = 'open' OR notes.status = 'open' AND notes.closed_at > '2022-08-22 21:21:59.933822') AND note_comments.author_id = 4062 ORDER BY updated_at DESC LIMIT 2;
