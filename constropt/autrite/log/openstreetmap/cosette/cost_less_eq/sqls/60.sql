...
-- Original Query
SELECT note_comments.* FROM note_comments LEFT OUTER JOIN users ON users.id = note_comments.author_id WHERE note_comments.note_id = 3380 AND note_comments.visible = False AND (users.status IN ('pending', 'pending') OR users.status IS NULL) ORDER BY note_comments.created_at ASC LIMIT 2;
-- Rewritten Queries
SELECT note_comments.* FROM note_comments LEFT OUTER JOIN users ON users.id = note_comments.author_id WHERE note_comments.note_id = 3380 AND note_comments.visible = False AND False ORDER BY note_comments.created_at ASC LIMIT 2;
SELECT note_comments.* FROM note_comments INNER JOIN users ON users.id = note_comments.author_id WHERE note_comments.note_id = 3380 AND note_comments.visible = False AND False ORDER BY note_comments.created_at ASC LIMIT 2;
SELECT note_comments.* FROM note_comments LEFT OUTER JOIN users ON users.id = note_comments.author_id WHERE note_comments.note_id = 3380 AND note_comments.visible = False AND users.status IS NULL ORDER BY note_comments.created_at ASC LIMIT 2;
