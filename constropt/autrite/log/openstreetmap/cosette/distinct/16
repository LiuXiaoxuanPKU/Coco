...
-- Original Query
SELECT COUNT(*) FROM (SELECT DISTINCT notes.* FROM notes INNER JOIN note_comments ON notes.id = note_comments.note_id WHERE note_comments.author_id = 9482 AND notes.status != 'open' ORDER BY updated_at DESC, id LIMIT 3 OFFSET 1 subquery_for_count
;
-- Rewritten Queries
SELECT COUNT(*) FROM (SELECT  notes.* FROM notes INNER JOIN note_comments ON notes.id = note_comments.note_id WHERE note_comments.author_id = 9482 AND notes.status != 'open' ORDER BY updated_at DESC, id LIMIT 3 OFFSET 1 subquery_for_count
