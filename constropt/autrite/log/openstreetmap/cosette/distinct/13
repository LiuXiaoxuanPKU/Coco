...
-- Original Query
SELECT DISTINCT notes.* FROM notes INNER JOIN note_comments ON notes.id = note_comments.note_id WHERE note_comments.author_id = 1033 AND notes.status != 'open' ORDER BY updated_at DESC, id LIMIT 10 OFFSET 1;
-- Rewritten Queries
SELECT  notes.* FROM notes INNER JOIN note_comments ON notes.id = note_comments.note_id WHERE note_comments.author_id = 1033 AND notes.status != 'open' ORDER BY updated_at DESC, id LIMIT 10 OFFSET 1