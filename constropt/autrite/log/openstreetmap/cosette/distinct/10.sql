...
-- Original Query
SELECT DISTINCT notes.* FROM notes INNER JOIN note_comments ON notes.id = note_comments.note_id WHERE note_comments.author_id = 3725 AND notes.status != 'open' LIMIT 5 OFFSET 1;
-- Rewritten Queries
SELECT  notes.* FROM notes INNER JOIN note_comments ON notes.id = note_comments.note_id WHERE note_comments.author_id = 3725 AND notes.status != 'open' LIMIT 5 OFFSET 1