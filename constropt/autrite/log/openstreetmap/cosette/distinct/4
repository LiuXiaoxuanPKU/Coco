...
-- Original Query
SELECT DISTINCT users.* FROM users INNER JOIN user_roles ON user_roles.user_id = users.id WHERE user_roles.role = 'moderator';
-- Rewritten Queries
SELECT  users.* FROM users INNER JOIN user_roles ON user_roles.user_id = users.id WHERE user_roles.role = 'moderator'