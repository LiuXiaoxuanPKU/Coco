...
-- Original Query
SELECT DISTINCT notes.* FROM notes WHERE (notes.status = 'open' OR notes.status = 'open' AND notes.closed_at > '2022-08-22 21:22:00.023387') AND notes.created_at BETWEEN $3 AND $4 ORDER BY updated_at DESC LIMIT 5;
-- Rewritten Queries
SELECT  notes.* FROM notes WHERE (notes.status = 'open' OR notes.status = 'open' AND notes.closed_at > '2022-08-22 21:22:00.023387') AND notes.created_at BETWEEN $3 AND $4 ORDER BY updated_at DESC LIMIT 5