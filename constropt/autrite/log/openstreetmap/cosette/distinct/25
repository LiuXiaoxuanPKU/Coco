...
-- Original Query
SELECT COUNT(*) FROM (SELECT DISTINCT gpx_files.id FROM gpx_files INNER JOIN gpx_file_tags ON gpx_file_tags.gpx_id = gpx_files.id LEFT OUTER JOIN users ON users.id = gpx_files.user_id WHERE gpx_files.user_id = 3665 AND gpx_files.visible = False AND gpx_file_tags.tag = 'mguuxlgzokcefnqfuaqygoyysdvrowtyxiydbioubzbshhivruzccsojaahrelghekzfqguagurqhgfmeuffkhjepmdlp' AND gpx_files.visible = False AND gpx_files.id = 1272 subquery_for_count
;
-- Rewritten Queries
