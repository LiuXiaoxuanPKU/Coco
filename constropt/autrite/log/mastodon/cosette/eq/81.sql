...
-- Original Query
SELECT COALESCE(NULLIF(accounts.shared_inbox_url, ''), accounts.inbox_url) AS preferred_inbox_url FROM accounts INNER JOIN blocks ON accounts.id = blocks.account_id WHERE blocks.target_account_id = 108847820748993043 AND accounts.protocol = 0 GROUP BY preferred_inbox_url;
-- Rewritten Queries
SELECT COALESCE(NULLIF(accounts.shared_inbox_url, ''), accounts.inbox_url) AS preferred_inbox_url FROM accounts INNER JOIN blocks ON accounts.id = blocks.account_id WHERE blocks.target_account_id = 108847820748993043 AND accounts.protocol = 0 GROUP BY preferred_inbox_url LIMIT 1;
