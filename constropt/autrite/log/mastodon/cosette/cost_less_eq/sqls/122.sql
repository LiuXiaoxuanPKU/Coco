...
SELECT mentions.id, mentions.account_id FROM mentions INNER JOIN accounts ON accounts.id = mentions.account_id INNER JOIN users ON users.account_id = accounts.id WHERE mentions.status_id = 107299520714125243 AND accounts.domain IS NULL AND users.current_sign_in_at > '2022-07-30 06:41:06.925285' ORDER BY mentions.id ASC LIMIT 1;
