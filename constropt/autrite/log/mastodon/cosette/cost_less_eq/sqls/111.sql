...
-- Original Query
SELECT accounts.id FROM accounts INNER JOIN follows ON accounts.id = follows.account_id INNER JOIN users ON users.account_id = accounts.id WHERE follows.target_account_id = 108847814383253791 AND accounts.domain IS NULL AND users.current_sign_in_at > '2022-07-30 06:43:03.880500' ORDER BY accounts.id ASC LIMIT 2;
-- Rewritten Queries
