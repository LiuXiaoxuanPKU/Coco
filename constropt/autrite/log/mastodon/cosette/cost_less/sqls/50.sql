...
SELECT mentions.id, mentions.account_id FROM mentions INNER JOIN accounts ON accounts.id = mentions.account_id INNER JOIN users ON users.account_id = accounts.id WHERE mentions.status_id = 104341690014054958 AND accounts.domain IS NULL AND users.current_sign_in_at > '2022-07-30 06:41:34.258204' ORDER BY mentions.id ASC LIMIT 6;
