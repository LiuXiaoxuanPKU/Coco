...
-- Original Query
SELECT accounts.id FROM accounts INNER JOIN follows ON accounts.id = follows.account_id INNER JOIN users ON users.account_id = accounts.id WHERE follows.target_account_id = 108847815190982247 AND accounts.domain IS NULL AND users.current_sign_in_at > '2022-07-30 06:39:54.495879' ORDER BY accounts.id ASC LIMIT 4;
-- Rewritten Queries
