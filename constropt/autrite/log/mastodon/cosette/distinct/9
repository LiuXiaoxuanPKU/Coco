...
-- Original Query
SELECT DISTINCT statuses.id AS alias_0, accounts.id FROM accounts LEFT OUTER JOIN statuses ON statuses.deleted_at IS NULL AND statuses.account_id = accounts.id LEFT OUTER JOIN account_stats ON account_stats.account_id = accounts.id WHERE accounts.suspended_at IS NULL AND 1=1 AND statuses.deleted_at IS NULL AND statuses.reblog_of_id = 106286968632774572 AND statuses.visibility IN (1, 2) ORDER BY statuses.id DESC LIMIT 5;
-- Rewritten Queries
SELECT  statuses.id AS alias_0, accounts.id FROM accounts LEFT OUTER JOIN statuses ON statuses.deleted_at IS NULL AND statuses.account_id = accounts.id LEFT OUTER JOIN account_stats ON account_stats.account_id = accounts.id WHERE accounts.suspended_at IS NULL AND 1=1 AND statuses.deleted_at IS NULL AND statuses.reblog_of_id = 106286968632774572 AND statuses.visibility IN (1, 2) ORDER BY statuses.id DESC LIMIT 5