...
-- Original Query
SELECT DISTINCT follows.id AS alias_0, accounts.id FROM accounts LEFT OUTER JOIN follows ON follows.target_account_id = accounts.id LEFT OUTER JOIN account_stats ON account_stats.account_id = accounts.id WHERE follows.account_id = 108847806636080415 ORDER BY follows.id DESC LIMIT 1;
-- Rewritten Queries
SELECT  follows.id AS alias_0, accounts.id FROM accounts LEFT OUTER JOIN follows ON follows.target_account_id = accounts.id LEFT OUTER JOIN account_stats ON account_stats.account_id = accounts.id WHERE follows.account_id = 108847806636080415 ORDER BY follows.id DESC LIMIT 1