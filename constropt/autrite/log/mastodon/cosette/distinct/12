...
-- Original Query
SELECT COUNT(*) FROM (SELECT DISTINCT accounts.id FROM accounts INNER JOIN follows ON accounts.id = follows.account_id LEFT OUTER JOIN account_stats ON account_stats.account_id = accounts.id WHERE follows.target_account_id = 108847824115937655 subquery_for_count
;
-- Rewritten Queries
SELECT COUNT(*) FROM (SELECT  accounts.id FROM accounts INNER JOIN follows ON accounts.id = follows.account_id LEFT OUTER JOIN account_stats ON account_stats.account_id = accounts.id WHERE follows.target_account_id = 108847824115937655 subquery_for_count
