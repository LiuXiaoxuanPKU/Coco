...
-- Original Query
SELECT 1 AS "one" FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 803 AND projects.status <> 9 AND projects.id = 2644 LIMIT 6;
-- Rewritten Queries
SELECT 1 AS "one" FROM projects WHERE projects.status <> 9 AND projects.id = 2644 LIMIT 6;
