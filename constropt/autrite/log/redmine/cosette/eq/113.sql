...
-- Rewritten Queries
SELECT users.* FROM users INNER JOIN members ON members.user_id = users.id WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND members.project_id = 1 ORDER BY users.firstname, users.lastname, users.id;
SELECT users.* FROM users INNER JOIN members ON members.user_id = users.id WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND members.project_id = 1 ORDER BY users.firstname, users.lastname, users.id LIMIT 1;
