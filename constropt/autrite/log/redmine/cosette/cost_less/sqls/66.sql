...
-- Original Query
SELECT projects.id, projects.name, projects.identifier, projects.lft, projects.rgt FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 2647 AND projects.status <> 9 AND projects.status = 9;
-- Rewritten Queries
