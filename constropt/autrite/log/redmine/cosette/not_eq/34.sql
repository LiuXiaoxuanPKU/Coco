...
-- Original Query
SELECT members.* FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 481 AND users.type = 'User' AND users.status = 2;
-- Rewritten Queries
SELECT members.* FROM members WHERE members.project_id = 481;
SELECT members.* FROM members WHERE members.project_id = 481 LIMIT 1;
