...
SELECT issues.* FROM issues INNER JOIN trackers ON trackers.id = issues.tracker_id INNER JOIN projects ON projects.id = issues.project_id WHERE issues.status_id IN (SELECT id FROM issue_statuses WHERE is_closed = False) AND issues.due_date > '2022-02-09 23:59:59.999999' AND projects.lft >= 1 AND projects.rgt <= 10;
...
SELECT issues.* FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE issues.status_id IN (SELECT id FROM issue_statuses WHERE is_closed = False) AND issues.due_date > '2022-02-09 23:59:59.999999' AND projects.lft >= 1 AND projects.rgt <= 10;
