...
-- Original Query
SELECT 1 AS "one" FROM users INNER JOIN groups_users ON users.id = groups_users.user_id WHERE users.type IN ('User', 'AnonymousUser') AND groups_users.group_id = 11 LIMIT 5;
-- Rewritten Queries
SELECT 1 AS "one" FROM users WHERE users.type IN ('User', 'AnonymousUser') LIMIT 5;
