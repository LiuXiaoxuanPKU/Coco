...
-- Original Query
SELECT 1 AS "one" FROM users INNER JOIN groups_users ON users.id = groups_users.user_id WHERE users.type IN ('User', 'AnonymousUser') AND groups_users.group_id = 10 AND users.id = 293 LIMIT 6;
-- Rewritten Queries
SELECT 1 AS "one" FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.id = 293 LIMIT 6;
