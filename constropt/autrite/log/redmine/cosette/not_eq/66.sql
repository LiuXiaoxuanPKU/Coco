...
-- Original Query
SELECT 1 AS "one" FROM users INNER JOIN members ON members.user_id = users.id WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND members.project_id = 1 LIMIT 2;
-- Rewritten Queries
SELECT 1 AS "one" FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 LIMIT 2;
