...
-- Original Query
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_from_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13)) AND issues.is_private = False AND issue_relations.issue_to_id IN (2919, 9341, 1750, 201, 9042, 9890);
-- Rewritten Queries
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_from_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking' LIMIT 1) IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13)) AND issues.is_private = False AND issue_relations.issue_to_id IN (1750, 201, 9042, 9890, 7991, 328);
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_from_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13) LIMIT 1) AND issues.is_private = False AND issue_relations.issue_to_id IN (1750, 201, 9042, 9890, 7991, 328);
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_from_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking' LIMIT 1) IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13) LIMIT 1) AND issues.is_private = False AND issue_relations.issue_to_id IN (9042, 9890, 7991, 328, 9683, 4588);
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_from_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking' LIMIT 1) IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13) LIMIT 1) AND issues.is_private = False AND issue_relations.issue_to_id IN (9042, 9890, 7991, 328, 9683, 4588) LIMIT 1;
