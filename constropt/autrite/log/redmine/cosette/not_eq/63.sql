...
-- Original Query
SELECT members.* FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 4396 AND users.type = 'User' AND users.status = 2 ORDER BY members.id ASC LIMIT 5;
-- Rewritten Queries
SELECT members.* FROM members WHERE members.project_id = 4396 ORDER BY members.id ASC LIMIT 5;
