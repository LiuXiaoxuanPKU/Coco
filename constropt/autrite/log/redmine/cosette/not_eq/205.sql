...
-- Rewritten Queries
SELECT issues.* FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id INNER JOIN trackers ON trackers.id = issues.tracker_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.id IN (1, 3);
SELECT issues.* FROM issues INNER JOIN trackers ON trackers.id = issues.tracker_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.id IN (1, 3);
SELECT issues.* FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.id IN (1, 3);
SELECT issues.* FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE projects.id IN (1, 3);
