...
-- Original Query
SELECT projects.* FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 2921 AND projects.status <> 9;
-- Rewritten Queries
SELECT projects.* FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 2921 AND projects.status <> 9 LIMIT 1;
