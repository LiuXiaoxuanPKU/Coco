...
-- Original Query
SELECT changesets.* FROM changesets INNER JOIN repositories ON repositories.id = changesets.repository_id INNER JOIN projects ON projects.id = repositories.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'repository') IS NOT NULL AND changesets.repository_id = 762 AND changesets.revision = 'gbdzruojihpeardjafmxtxedvmpvpbpcgozknkeklfjlgbtzhehnoqqxzjddmjzhgnclgffwzkfxjleudndpzrpjbvcuojgiiplwgjbfpi' LIMIT 1;
-- Rewritten Queries
SELECT changesets.* FROM changesets INNER JOIN repositories ON repositories.id = changesets.repository_id INNER JOIN projects ON projects.id = repositories.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'repository' LIMIT 1) IS NOT NULL AND changesets.repository_id = 762 AND changesets.revision = 'gbdzruojihpeardjafmxtxedvmpvpbpcgozknkeklfjlgbtzhehnoqqxzjddmjzhgnclgffwzkfxjleudndpzrpjbvcuojgiiplwgjbfpi' LIMIT 1;
