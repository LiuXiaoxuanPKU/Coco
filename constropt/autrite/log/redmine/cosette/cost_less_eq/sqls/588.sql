...
-- Rewritten Queries
SELECT queries.* FROM queries WHERE queries.type IN ('ProjectQuery') AND queries.project_id IS NULL AND queries.visibility = 2 AND queries.project_id IS NULL ORDER BY queries.name ASC, queries.id ASC;
SELECT queries.* FROM queries WHERE queries.type IN ('ProjectQuery') AND queries.project_id IS NULL AND queries.visibility = 2 AND queries.project_id IS NULL ORDER BY queries.name ASC, queries.id ASC LIMIT 1;
