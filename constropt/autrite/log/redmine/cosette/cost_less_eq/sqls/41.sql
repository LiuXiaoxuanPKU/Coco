...
-- Original Query
SELECT COUNT(*) FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 6098 AND users.type = 'User' AND users.status = 2;
-- Rewritten Queries
SELECT COUNT(*) FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 6098 AND users.type = 'User' AND users.status = 2 LIMIT 1;
