...
-- Original Query
SELECT members.* FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 7779 AND users.type = 'User' AND users.status = 1 ORDER BY members.id ASC LIMIT 6;
-- Rewritten Queries
SELECT members.* FROM members WHERE members.project_id = 7779 ORDER BY members.id ASC LIMIT 6;
