...
-- Original Query
SELECT projects.id, projects.name, projects.identifier, projects.lft, projects.rgt FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 4089 AND projects.status <> 1 AND projects.status = 1;
-- Rewritten Queries
SELECT projects.id, projects.name, projects.identifier, projects.lft, projects.rgt FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 4089 AND projects.status <> 1 AND projects.status = 1 LIMIT 1;
