...
-- Original Query
SELECT notifications.* FROM notifications WHERE notifications.organization_id = 2 AND notifications.user_id IS NULL ORDER BY notifications.notified_at DESC LIMIT 3;
-- Rewritten Queries
SELECT notifications.* FROM notifications WHERE notifications.organization_id = 2 AND False ORDER BY notifications.notified_at DESC LIMIT 3;
