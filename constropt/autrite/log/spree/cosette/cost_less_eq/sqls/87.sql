...
-- Original Query
SELECT spree_products.* FROM spree_products INNER JOIN spree_product_properties ON spree_product_properties.product_id = spree_products.id INNER JOIN spree_properties ON spree_properties.id = spree_product_properties.property_id WHERE spree_products.deleted_at IS NULL AND spree_properties.filter_param = 'zcomlcoacyrqibzxychgesgetisddqdccucvnbzedsruopiywbaeubwmqzdpeuijfdnvncdalczphlsdxaxipfmiphbltuijcjwzyobwhzfpsxijyuhkgizxzyglexoqdrwgdmewozjxgkpttfzbabjvyqahwzssfzwjzoznurkqbdqetymgoxtdlqfqqwdxvzdeyeymzxosufbtmnwnupnruvnqtaxfgdwplllrxgxsfhs' AND spree_product_properties.filter_param IN ('eo', 'torfsmtt', 'knxnkoemtsddn');
-- Rewritten Queries
SELECT spree_products.* FROM spree_products INNER JOIN spree_product_properties ON spree_product_properties.product_id = spree_products.id INNER JOIN spree_properties ON spree_properties.id = spree_product_properties.property_id WHERE spree_products.deleted_at IS NULL AND spree_properties.filter_param = 'zcomlcoacyrqibzxychgesgetisddqdccucvnbzedsruopiywbaeubwmqzdpeuijfdnvncdalczphlsdxaxipfmiphbltuijcjwzyobwhzfpsxijyuhkgizxzyglexoqdrwgdmewozjxgkpttfzbabjvyqahwzssfzwjzoznurkqbdqetymgoxtdlqfqqwdxvzdeyeymzxosufbtmnwnupnruvnqtaxfgdwplllrxgxsfhs' AND spree_product_properties.filter_param IN ('eo', 'torfsmtt', 'knxnkoemtsddn') LIMIT 1;
