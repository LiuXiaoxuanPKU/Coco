...
-- Original Query
SELECT DISTINCT spree_promotions.id AS alias_0, spree_promotions.id FROM spree_promotions INNER JOIN spree_promotions_stores ON spree_promotions.id = spree_promotions_stores.promotion_id LEFT OUTER JOIN spree_promotion_rules ON spree_promotion_rules.promotion_id = spree_promotions.id LEFT OUTER JOIN spree_promotion_actions ON spree_promotion_actions.deleted_at IS NULL AND spree_promotion_actions.promotion_id = spree_promotions.id WHERE spree_promotions_stores.store_id = 10779 AND (spree_promotions.starts_at IS NULL OR spree_promotions.starts_at < '2022-08-14 05:29:19.280386') AND (spree_promotions.expires_at IS NULL OR spree_promotions.expires_at > '2022-08-14 05:29:19.280553') AND LOWER(spree_promotions.code) = '10off' AND spree_promotion_actions.id IS NOT NULL ORDER BY spree_promotions.id DESC LIMIT 6;
-- Rewritten Queries
