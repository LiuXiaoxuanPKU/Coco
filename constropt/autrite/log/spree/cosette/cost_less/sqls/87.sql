...
-- Rewritten Queries
SELECT DISTINCT spree_zones.id FROM spree_zones INNER JOIN spree_zone_members ON spree_zone_members.zoneable_type = 'Spree::Country' AND spree_zone_members.zone_id = spree_zones.id INNER JOIN spree_countries ON spree_countries.id = spree_zone_members.zoneable_id INNER JOIN spree_zone_members AS zone_members_spree_countries_join ON zone_members_spree_countries_join.zoneable_type = 'Spree::Country' AND zone_members_spree_countries_join.zoneable_id = spree_countries.id WHERE zone_members_spree_countries_join.zone_id = 390;
