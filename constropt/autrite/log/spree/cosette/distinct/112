...
-- Original Query
SELECT COUNT(count_column) FROM (SELECT DISTINCT spree_products.id AS count_column FROM spree_products INNER JOIN spree_products_stores ON spree_products.id = spree_products_stores.product_id INNER JOIN spree_variants ON spree_variants.deleted_at IS NULL AND spree_variants.product_id = spree_products.id INNER JOIN spree_prices ON spree_prices.deleted_at IS NULL AND spree_prices.variant_id = spree_variants.id INNER JOIN spree_variants masters_spree_products ON masters_spree_products.deleted_at IS NULL AND masters_spree_products.is_master = True AND masters_spree_products.product_id = spree_products.id INNER JOIN spree_prices default_prices_spree_variants ON default_prices_spree_variants.currency = 'usd' AND default_prices_spree_variants.variant_id = masters_spree_products.id WHERE spree_products.deleted_at IS NULL AND spree_products_stores.store_id = 8603 AND spree_prices.currency = 'usd' AND spree_prices.amount IS NOT NULL AND (spree_products.deleted_at IS NULL or spree_products.deleted_at >= '2022-08-14 05:42:06.412125') AND (spree_products.discontinue_on IS NULL or spree_products.discontinue_on >= '2022-08-14 05:42:06.412255') AND (spree_products.available_on <= '2022-08-14 05:42:06.412249') AND spree_prices.currency = 'usd' AND spree_prices.amount IS NOT NULL AND (spree_prices.amount <= 10.0 OR spree_prices.amount BETWEEN 10.0 AND 15.0) LIMIT 3 OFFSET 1 subquery_for_count
;
-- Rewritten Queries
