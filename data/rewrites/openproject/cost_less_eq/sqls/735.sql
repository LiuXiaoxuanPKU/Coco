...
-- Original Query
SELECT 1 AS "one" FROM projects INNER JOIN enabled_modules ON projects.id = enabled_modules.project_id AND enabled_modules.name IN ('work_package_tracking') AND projects.active = True WHERE projects.active = True LIMIT 1;
-- Rewritten Queries
SELECT 1 AS "one" FROM projects WHERE projects.active = True LIMIT 1;
