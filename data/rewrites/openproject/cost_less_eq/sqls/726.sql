...
-- Original Query
SELECT 1 AS "one" FROM relations WHERE relations.from_id IS NULL AND relations.to_id = 7253 AND NOT hierarchy + relates + duplicates + follows + blocks + includes + requires > 1 LIMIT 1;
-- Rewritten Queries
SELECT 1 AS "one" FROM relations WHERE False AND relations.to_id = 7253 AND NOT hierarchy + relates + duplicates + follows + blocks + includes + requires > 1 LIMIT 1;
