...
-- Original Query
SELECT credits.purchase_id, credits.purchase_type, COUNT(*) AS cost, MAX(spent_at) AS purchased_at FROM credits WHERE credits.user_id = 7026 AND credits.spent = False GROUP BY credits.purchase_id, credits.purchase_type ORDER BY purchased_at DESC;
-- Rewritten Queries
SELECT credits.purchase_id, credits.purchase_type, COUNT(*) AS cost, MAX(spent_at) AS purchased_at FROM credits WHERE credits.user_id = 7026 AND credits.spent = False GROUP BY credits.purchase_id, credits.purchase_type ORDER BY purchased_at DESC LIMIT 1;
