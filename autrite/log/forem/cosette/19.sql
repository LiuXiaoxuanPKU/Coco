...
-- Original Query
SELECT 1 AS "one" FROM users WHERE (id NOT IN (SELECT user_id FROM users_roles WHERE role_id IN (SELECT roles.id FROM roles WHERE roles.name IN ('comment_suspended', 'suspended', 'trusted', 'warned')))) LIMIT 6 OFFSET 1;
-- Rewritten Queries
SELECT 1 AS "one" FROM users WHERE id NOT IN (SELECT user_id FROM users_roles WHERE role_id IN (SELECT roles.id FROM roles WHERE roles.name IN ('comment_suspended', 'suspended', 'trusted', 'warned')) LIMIT 1) LIMIT 6 OFFSET 1;
SELECT 1 AS "one" FROM users WHERE id NOT IN (SELECT user_id FROM users_roles WHERE role_id IN (SELECT roles.id FROM roles WHERE roles.name IN ('comment_suspended', 'suspended', 'trusted', 'warned') LIMIT 1) LIMIT 1) LIMIT 6 OFFSET 1;
SELECT 1 AS "one" FROM users WHERE id NOT IN (SELECT user_id FROM users_roles WHERE role_id IN (SELECT roles.id FROM roles WHERE roles.name IN ('comment_suspended', 'suspended', 'trusted', 'warned') LIMIT 1)) LIMIT 6 OFFSET 1;
