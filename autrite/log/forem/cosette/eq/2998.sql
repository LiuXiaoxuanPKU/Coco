...
-- Original Query
SELECT roles.* FROM roles INNER JOIN users_roles ON roles.id = users_roles.role_id WHERE users_roles.user_id = 453 AND (roles.name = 'tag_moderator' AND roles.resource_type IS NULL AND roles.resource_id IS NULL OR roles.name = 'tag_moderator' AND roles.resource_type = 'Tag' AND roles.resource_id IS NULL OR roles.name = 'tag_moderator' AND roles.resource_type = 'Tag' AND roles.resource_id = 3862);
-- Rewritten Queries
SELECT roles.* FROM roles INNER JOIN users_roles ON roles.id = users_roles.role_id WHERE users_roles.user_id = 453 AND (roles.name = 'tag_moderator' AND roles.resource_type IS NULL AND roles.resource_id IS NULL OR roles.name = 'tag_moderator' AND roles.resource_type = 'Tag' AND roles.resource_id IS NULL OR roles.name = 'tag_moderator' AND roles.resource_type = 'Tag' AND roles.resource_id = 3862) LIMIT 1;
