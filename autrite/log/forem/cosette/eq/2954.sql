...
-- Original Query
SELECT COUNT(*) FROM roles INNER JOIN users_roles ON roles.id = users_roles.role_id WHERE users_roles.user_id = 758 AND (roles.name = 'tech_admin' AND roles.resource_type IS NULL AND roles.resource_id IS NULL OR roles.name = 'super_admin' AND roles.resource_type IS NULL AND roles.resource_id IS NULL);
-- Rewritten Queries
SELECT COUNT(*) FROM roles INNER JOIN users_roles ON roles.id = users_roles.role_id WHERE users_roles.user_id = 758 AND (roles.name = 'tech_admin' AND roles.resource_type IS NULL AND roles.resource_id IS NULL OR roles.name = 'super_admin' AND roles.resource_type IS NULL AND roles.resource_id IS NULL) LIMIT 1;
