...
-- Original Query
SELECT roles.* FROM roles INNER JOIN users_roles ON roles.id = users_roles.role_id WHERE users_roles.user_id = 1482 AND (roles.name = 'single_resource_admin' AND roles.resource_type IS NULL AND roles.resource_id IS NULL OR roles.name = 'single_resource_admin' AND roles.resource_type = 'DataUpdateScript' AND roles.resource_id IS NULL);
-- Rewritten Queries
SELECT roles.* FROM roles INNER JOIN users_roles ON roles.id = users_roles.role_id WHERE users_roles.user_id = 1482 AND (roles.name = 'single_resource_admin' AND roles.resource_type IS NULL AND roles.resource_id IS NULL OR roles.name = 'single_resource_admin' AND roles.resource_type = 'DataUpdateScript' AND roles.resource_id IS NULL) LIMIT 1;
