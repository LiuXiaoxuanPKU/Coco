...
-- Original Query
SELECT 1 AS "one" FROM notifications WHERE notifications.user_id = 3418 AND notifications.organization_id IS NULL AND notifications.notifiable_id IS NULL AND notifications.notifiable_type = 'Article' AND notifications.action IS NULL LIMIT 1;
-- Rewritten Queries
SELECT 1 AS "one" FROM notifications WHERE notifications.user_id = 3418 AND False AND notifications.notifiable_id IS NULL AND notifications.notifiable_type = 'Article' AND notifications.action IS NULL LIMIT 1;
