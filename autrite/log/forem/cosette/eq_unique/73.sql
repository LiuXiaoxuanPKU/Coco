...
-- Original Query
SELECT 1 AS "one" FROM notifications WHERE notifications.user_id IS NULL AND notifications.organization_id = 2 AND notifications.notifiable_id = 6866 AND notifications.notifiable_type = 'TagAdjustment' AND notifications.action IS NULL LIMIT 7;
-- Rewritten Queries
SELECT 1 AS "one" FROM notifications WHERE False AND notifications.organization_id = 2 AND notifications.notifiable_id = 6866 AND notifications.notifiable_type = 'TagAdjustment' AND notifications.action IS NULL LIMIT 7;
