...
-- Original Query
SELECT users.id, users.id, COUNT(badge_achievements.id) * 1 AS count, MAX(users.badge_achievements_count) AS badge_achievements_count FROM users LEFT JOIN badge_achievements AS badge_achievements ON users.id = badge_achievements.user_id WHERE users.id = 9302 GROUP BY users.id ORDER BY users.id ASC LIMIT 9;
-- Rewritten Queries
SELECT users.id, users.id, COUNT(badge_achievements.id) * 1 AS count, MAX(users.badge_achievements_count) AS badge_achievements_count FROM users INNER JOIN badge_achievements AS badge_achievements ON users.id = badge_achievements.user_id WHERE users.id = 9302 GROUP BY users.id ORDER BY users.id ASC LIMIT 9;
