...
-- Original Query
SELECT notifications.* FROM notifications WHERE notifications.organization_id = 3 AND notifications.user_id IS NULL ORDER BY notifications.notified_at DESC LIMIT 8;
-- Rewritten Queries
SELECT notifications.* FROM notifications WHERE notifications.organization_id = 3 AND False ORDER BY notifications.notified_at DESC LIMIT 8;
