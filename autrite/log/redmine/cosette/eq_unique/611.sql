...
-- Original Query
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals INNER JOIN issues ON issues.id = journals.journalized_id INNER JOIN projects ON projects.id = issues.project_id WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (2478, 4065, 9910, 33, 5306, 2660, 4461, 6246, 2735, 1009, 4920) AND (journals.private_notes = False OR journals.user_id = 2 OR projects.status <> 9 AND projects.id IN (1, 5)) AND journals.notes <> '{}' GROUP BY journals.journalized_id;
-- Rewritten Queries
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (4188, 5734, 2023, 3822, 2964, 1584, 9057, 3934, 2467, 1613, 4458) AND (journals.private_notes = False OR journals.user_id = 2) AND journals.notes <> '{}' GROUP BY journals.journalized_id;
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals INNER JOIN issues ON issues.id = journals.journalized_id WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (3934, 2467, 1613, 4458, 9334, 8872, 9603, 2478, 4065, 9910, 33) AND (journals.private_notes = False OR journals.user_id = 2) AND journals.notes <> '{}' GROUP BY journals.journalized_id;
