...
-- Original Query
SELECT COUNT(issues.id) AS count_id, is_closed AS is_closed FROM issues INNER JOIN projects ON projects.id = issues.project_id INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issues.root_id = 6304 AND issues.lft > 1 AND issues.rgt < 4 AND projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13)) AND issues.is_private = False GROUP BY is_closed ORDER BY is_closed ASC;
-- Rewritten Queries
SELECT COUNT(issues.id) AS count_id, is_closed AS is_closed FROM issues INNER JOIN projects ON projects.id = issues.project_id INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issues.root_id = 6304 AND issues.lft > 1 AND issues.rgt < 4 AND projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13) LIMIT 1) AND issues.is_private = False GROUP BY is_closed ORDER BY is_closed ASC;
