...
-- Original Query
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals INNER JOIN issues ON issues.id = journals.journalized_id INNER JOIN projects ON projects.id = issues.project_id WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (67, 380, 9362, 2814, 9687, 4355, 8329, 4575, 1263, 670, 2106) AND (journals.private_notes = False OR journals.user_id = 2 OR projects.status <> 9 AND projects.id IN (1, 5)) AND journals.notes <> '{}' GROUP BY journals.journalized_id;
-- Rewritten Queries
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (1505, 550, 8178, 3980, 3474, 5953, 1620, 7620, 7547, 1620, 7933) AND (journals.private_notes = False OR journals.user_id = 2) AND journals.notes <> '{}' GROUP BY journals.journalized_id;
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals INNER JOIN issues ON issues.id = journals.journalized_id WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (7620, 7547, 1620, 7933, 75, 9965, 2808, 67, 380, 9362, 2814) AND (journals.private_notes = False OR journals.user_id = 2) AND journals.notes <> '{}' GROUP BY journals.journalized_id;
