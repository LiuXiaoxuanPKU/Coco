...
-- Original Query
SELECT  users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND (users.status <> 0) AND users.status = 2 AND (users.id IN (SELECT gu.user_id FROM groups_users gu WHERE gu.group_id = 10)) ORDER BY login ASC LIMIT 8 OFFSET 1;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 2 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10 LIMIT 1) ORDER BY login ASC LIMIT 8 OFFSET 1;
