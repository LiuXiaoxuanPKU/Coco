...
-- Original Query
SELECT projects.* FROM projects WHERE projects.status = 5 AND projects.id IN (SELECT em.project_id FROM enabled_modules AS em WHERE em.name = 'repository') AND projects.identifier = 'rcdvjayhwhmqmueysgaeshqxgmavaczkpgjtwdlrnizwsrhezdsmsmuvgbnkuygasdktuitzafsdzzxngwvcdkzcsl' LIMIT 8;
-- Rewritten Queries
SELECT projects.* FROM projects WHERE projects.status = 5 AND projects.id IN (SELECT em.project_id FROM enabled_modules AS em WHERE em.name = 'repository' LIMIT 1) AND projects.identifier = 'rcdvjayhwhmqmueysgaeshqxgmavaczkpgjtwdlrnizwsrhezdsmsmuvgbnkuygasdktuitzafsdzzxngwvcdkzcsl' LIMIT 8;
