...
-- Original Query
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND users.login = 'jciagwutlrfniaca';
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND users.login = 'jciagwutlrfniaca' LIMIT 1;
