...
-- Original Query
SELECT COUNT(DISTINCT time_entries.id) FROM time_entries INNER JOIN projects ON projects.id = time_entries.project_id INNER JOIN users ON users.id = time_entries.user_id AND users.type IN ('User', 'AnonymousUser') LEFT OUTER JOIN enumerations ON enumerations.id = time_entries.activity_id AND enumerations.type IN ('TimeEntryActivity') LEFT OUTER JOIN issues ON issues.id = time_entries.issue_id LEFT OUTER JOIN custom_values cf_73 ON cf_73.customized_type = 'TimeEntry' AND cf_73.customized_id = time_entries.id AND cf_73.custom_field_id = 73 AND (1=1) AND cf_73.value <> '' AND cf_73.id = (SELECT max(cf_73_2.id) FROM custom_values cf_73_2 WHERE cf_73_2.customized_type = cf_73.customized_type AND cf_73_2.customized_id = cf_73.customized_id AND cf_73_2.custom_field_id = cf_73.custom_field_id) WHERE (((projects.status <> 9 AND EXISTS (SELECT 1 AS "one" FROM enabled_modules em WHERE em.project_id = projects.id AND em.name='time_tracking')) AND ((projects.is_public = TRUE AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6,13))))));
-- Rewritten Queries
