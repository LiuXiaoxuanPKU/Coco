...
-- Original Query
SELECT custom_values.id FROM custom_values WHERE custom_values.custom_field_id = 3 AND (SELECT 1 FROM custom_values AS cve WHERE cve.custom_field_id = custom_values.custom_field_id AND cve.customized_type = custom_values.customized_type AND cve.customized_id = custom_values.customized_id AND cve.id > custom_values.id) IS NOT NULL;
-- Rewritten Queries
...
SELECT custom_values.id FROM custom_values WHERE custom_values.custom_field_id = 3 AND (SELECT 1 FROM custom_values AS cve WHERE cve.custom_field_id = custom_values.custom_field_id AND cve.customized_type = custom_values.customized_type AND cve.customized_id = custom_values.customized_id AND cve.id > custom_values.id LIMIT 1) IS NOT NULL;
