...
-- Original Query
SELECT versions.* FROM versions WHERE versions.project_id = 866 AND versions.status IN ('closed', 'closed');
-- Rewritten Queries
SELECT versions.* FROM versions WHERE versions.project_id = 866 AND versions.status IN ('closed', 'closed') LIMIT 1;
