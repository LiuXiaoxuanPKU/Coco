...
-- Rewritten Queries
SELECT 1 AS "one" FROM versions WHERE (versions.sharing = 'system' OR versions.sharing = 'tree' OR versions.sharing IN ('hierarchy', 'descendants') OR versions.sharing = 'hierarchy') AND versions.status = 'open' LIMIT 4;
