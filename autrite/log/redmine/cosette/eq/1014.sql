...
-- Original Query
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND (users.status <> 0) AND users.status = 2 AND (users.id IN (SELECT gu.user_id FROM groups_users gu WHERE gu.group_id = 10)) ORDER BY login ASC;
-- Rewritten Queries
