...
-- Original Query
SELECT changesets.* FROM changesets INNER JOIN repositories ON repositories.id = changesets.repository_id INNER JOIN projects ON projects.id = repositories.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'repository') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13)) AND changesets.repository_id = 2802 AND changesets.revision = 'mxslwbkdibuinlfwnsgahfmirhzwueynukfoaxsfolreqifeccomeewiogsrpwhynridrkwymltaugsulv' LIMIT 9;
-- Rewritten Queries
SELECT changesets.* FROM changesets INNER JOIN repositories ON repositories.id = changesets.repository_id INNER JOIN projects ON projects.id = repositories.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'repository') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13) LIMIT 1) AND changesets.repository_id = 2802 AND changesets.revision = 'mxslwbkdibuinlfwnsgahfmirhzwueynukfoaxsfolreqifeccomeewiogsrpwhynridrkwymltaugsulv' LIMIT 9;
