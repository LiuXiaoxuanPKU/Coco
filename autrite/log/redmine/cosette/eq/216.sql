...
-- Original Query
SELECT  users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND (users.id NOT IN (SELECT gu.user_id FROM groups_users gu WHERE gu.group_id = 70)) ORDER BY users.firstname, users.lastname, users.id LIMIT 3 OFFSET 1;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND users.id NOT IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 70 LIMIT 1) ORDER BY users.firstname, users.lastname, users.id LIMIT 3 OFFSET 1;
