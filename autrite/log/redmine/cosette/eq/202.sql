...
-- Original Query
SELECT  users.* FROM users WHERE users.status = 1 AND (users.id = 6 OR users.id IN (SELECT user_id FROM members WHERE project_id IN (1,3,4,6))) AND users.id = 3823 LIMIT 7;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.status = 1 AND (users.id = 6 OR users.id IN (SELECT user_id FROM members WHERE project_id IN (1, 3, 4, 6) LIMIT 1)) AND users.id = 3823 LIMIT 7;
