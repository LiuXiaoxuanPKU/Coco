...
-- Original Query
SELECT projects.* FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 5647 AND projects.status <> 5;
-- Rewritten Queries
SELECT projects.* FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 5647 AND projects.status <> 5 LIMIT 1;
