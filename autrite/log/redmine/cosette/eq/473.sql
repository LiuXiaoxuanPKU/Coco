...
-- Original Query
SELECT COUNT(*) AS count_all, COALESCE(cf_103.value, '') AS coalesce_cf_103_value FROM issues INNER JOIN projects ON projects.id = issues.project_id INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id LEFT OUTER JOIN custom_values AS cf_103 ON cf_103.customized_type = 'Issue' AND cf_103.customized_id = issues.id AND cf_103.custom_field_id = 103 AND 1 = 1 AND issues.tracker_id IN (SELECT tracker_id FROM custom_fields_trackers WHERE custom_field_id = 103) AND ((SELECT 1 FROM custom_fields AS ifa WHERE ifa.is_for_all = True AND ifa.id = 103) IS NOT NULL OR issues.project_id IN (SELECT project_id FROM custom_fields_projects WHERE custom_field_id = 103)) AND cf_103.value <> '' AND cf_103.id = (SELECT MAX(cf_103_2.id) FROM custom_values AS cf_103_2 WHERE cf_103_2.customized_type = cf_103.customized_type AND cf_103_2.customized_id = cf_103.customized_id AND cf_103_2.custom_field_id = cf_103.custom_field_id) WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13)) AND issues.is_private = False AND issues.status_id IN (SELECT id FROM issue_statuses WHERE is_closed = False) AND 1 = 1 AND issues.tracker_id IN (SELECT tracker_id FROM custom_fields_trackers WHERE custom_field_id = 103) AND ((SELECT 1 FROM custom_fields AS ifa WHERE ifa.is_for_all = True AND ifa.id = 103) IS NOT NULL OR issues.project_id IN (SELECT project_id FROM custom_fields_projects WHERE custom_field_id = 103)) AND projects.lft >= 1 AND projects.rgt <= 10 GROUP BY COALESCE(cf_103.value, '');
-- Rewritten Queries
SELECT COUNT(*) AS count_all, COALESCE(cf_103.value, '') AS coalesce_cf_103_value FROM issues INNER JOIN projects ON projects.id = issues.project_id INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id INNER JOIN custom_values AS cf_103 ON cf_103.customized_type = 'Issue' AND cf_103.customized_id = issues.id AND cf_103.custom_field_id = 103 AND 1 = 1 AND issues.tracker_id IN (SELECT tracker_id FROM custom_fields_trackers WHERE custom_field_id = 103) AND ((SELECT 1 FROM custom_fields AS ifa WHERE ifa.is_for_all = True AND ifa.id = 103) IS NOT NULL OR issues.project_id IN (SELECT project_id FROM custom_fields_projects WHERE custom_field_id = 103)) AND cf_103.value <> '' AND cf_103.id = (SELECT MAX(cf_103_2.id) FROM custom_values AS cf_103_2 WHERE cf_103_2.customized_type = cf_103.customized_type AND cf_103_2.customized_id = cf_103.customized_id AND cf_103_2.custom_field_id = cf_103.custom_field_id) WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13)) AND issues.is_private = False AND issues.status_id IN (SELECT id FROM issue_statuses WHERE is_closed = False) AND 1 = 1 AND issues.tracker_id IN (SELECT tracker_id FROM custom_fields_trackers WHERE custom_field_id = 103) AND ((SELECT 1 FROM custom_fields AS ifa WHERE ifa.is_for_all = True AND ifa.id = 103) IS NOT NULL OR issues.project_id IN (SELECT project_id FROM custom_fields_projects WHERE custom_field_id = 103)) AND projects.lft >= 1 AND projects.rgt <= 10 GROUP BY COALESCE(cf_103.value, '');
