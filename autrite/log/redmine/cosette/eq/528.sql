...
-- Original Query
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 1 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10) ORDER BY login ASC;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 1 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10 LIMIT 1) ORDER BY login ASC;
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 1 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10 LIMIT 1) ORDER BY login ASC LIMIT 1;
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 1 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10) ORDER BY login ASC LIMIT 1;
