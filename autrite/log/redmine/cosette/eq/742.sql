...
-- Original Query
SELECT issues.* FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE issues.author_id = 2 AND projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL ORDER BY issues.id DESC LIMIT 7;
-- Rewritten Queries
SELECT issues.* FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE issues.author_id = 2 AND projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking' LIMIT 1) IS NOT NULL ORDER BY issues.id DESC LIMIT 7;
