...
-- Original Query
SELECT COUNT(*) FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 1 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10);
-- Rewritten Queries
SELECT COUNT(*) FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 1 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10 LIMIT 1);
SELECT COUNT(*) FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 1 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10 LIMIT 1) LIMIT 1;
SELECT COUNT(*) FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status <> 0 AND users.status = 1 AND users.id IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 10) LIMIT 1;
