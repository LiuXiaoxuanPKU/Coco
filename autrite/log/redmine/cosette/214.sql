...
-- Original Query
SELECT  users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 1 AND (users.id NOT IN (SELECT gu.user_id FROM groups_users gu WHERE gu.group_id = 15)) ORDER BY users.firstname, users.lastname, users.id LIMIT 4 OFFSET 1;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 1 AND users.id NOT IN (SELECT gu.user_id FROM groups_users AS gu WHERE gu.group_id = 15 LIMIT 1) ORDER BY users.firstname, users.lastname, users.id LIMIT 4 OFFSET 1;
