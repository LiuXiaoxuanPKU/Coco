...
-- Original Query
SELECT members.* FROM members INNER JOIN projects ON projects.id = members.project_id WHERE members.user_id = 1037 AND projects.status != 5 AND (((projects.status <> 9) AND ((projects.is_public = TRUE AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (140,12))))));
-- Rewritten Queries
SELECT members.* FROM members WHERE members.user_id = 1037;
SELECT members.* FROM members WHERE members.user_id = 1037 LIMIT 1;
