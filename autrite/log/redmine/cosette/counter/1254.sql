...
-- Original Query
SELECT users.* FROM users WHERE users.type IN ('User', 'User') ORDER BY users.type DESC, users.firstname, users.lastname, users.id;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'User') ORDER BY users.type DESC, users.firstname, users.lastname, users.id LIMIT 1;
