...
-- Original Query
SELECT  issues.* FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issue_statuses.is_closed = True ORDER BY issues.id ASC LIMIT 9;
-- Rewritten Queries
SELECT issues.* FROM issues ORDER BY issues.id ASC LIMIT 9;
