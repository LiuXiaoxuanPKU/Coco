...
-- Original Query
SELECT issues.id FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE ((projects.lft >= 2 AND projects.rgt <= 5));
-- Rewritten Queries
SELECT issues.id FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE projects.lft >= 2 AND projects.rgt <= 5 LIMIT 1;
