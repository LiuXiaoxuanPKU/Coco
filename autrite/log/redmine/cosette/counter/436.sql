...
-- Rewritten Queries
SELECT users.* FROM users WHERE users.status = 2 AND users.status = 2 AND users.status = 2 AND users.type IN ('User', 'User') ORDER BY users.type DESC, users.firstname, users.lastname, users.id LIMIT 1;
