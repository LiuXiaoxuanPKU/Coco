...
-- Original Query
SELECT  trackers.* FROM trackers INNER JOIN projects_trackers ON trackers.id = projects_trackers.tracker_id WHERE projects_trackers.project_id = 7074 AND trackers.id = 913 ORDER BY trackers.position ASC LIMIT 8;
-- Rewritten Queries
SELECT trackers.* FROM trackers WHERE trackers.id = 913 ORDER BY trackers.position ASC LIMIT 8;
