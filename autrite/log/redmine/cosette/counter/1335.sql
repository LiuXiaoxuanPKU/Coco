...
-- Original Query
SELECT versions.* FROM versions ORDER BY (CASE WHEN versions.effective_date IS NULL THEN 1 ELSE 0 END), versions.effective_date, versions.name, versions.id;
-- Rewritten Queries
SELECT versions.* FROM versions ORDER BY (CASE WHEN versions.effective_date IS NULL THEN 1 ELSE 0 END), versions.effective_date, versions.name, versions.id LIMIT 1;
