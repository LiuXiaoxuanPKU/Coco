...
-- Original Query
SELECT users.id FROM users INNER JOIN groups_users ON users.id = groups_users.user_id WHERE users.type IN ('User', 'AnonymousUser') AND groups_users.group_id = 10;
-- Rewritten Queries
SELECT users.id FROM users WHERE users.type IN ('User', 'AnonymousUser') LIMIT 1;
