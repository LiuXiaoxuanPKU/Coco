...
-- Original Query
SELECT trackers.* FROM trackers INNER JOIN projects_trackers ON trackers.id = projects_trackers.tracker_id WHERE projects_trackers.project_id = 8877 AND trackers.id IN (6820, 1801) ORDER BY trackers.position ASC LIMIT 7;
-- Rewritten Queries
SELECT trackers.* FROM trackers WHERE trackers.id IN (7065, 9132) ORDER BY trackers.position ASC LIMIT 7;
