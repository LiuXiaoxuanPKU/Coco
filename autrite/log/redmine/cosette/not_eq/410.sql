...
-- Original Query
SELECT DISTINCT roles.id FROM roles INNER JOIN member_roles ON roles.id = member_roles.role_id WHERE member_roles.member_id = 425;
-- Rewritten Queries
SELECT DISTINCT roles.id FROM roles LIMIT 1;
SELECT roles.id FROM roles LIMIT 1;
