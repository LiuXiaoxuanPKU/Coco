...
-- Original Query
SELECT projects.id FROM projects INNER JOIN members ON projects.id = members.project_id WHERE members.user_id = 5253 AND projects.status <> 1;
-- Rewritten Queries
SELECT projects.id FROM projects WHERE projects.status <> 1 LIMIT 1;
