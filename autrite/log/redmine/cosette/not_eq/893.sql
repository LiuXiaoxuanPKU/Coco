...
-- Original Query
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_to_id INNER JOIN projects ON projects.id = issues.project_id WHERE (((projects.status <> 9 AND EXISTS (SELECT 1 AS "one" FROM enabled_modules em WHERE em.project_id = projects.id AND em.name='issue_tracking')) AND (((projects.is_public = TRUE AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6,13))) AND ((issues.is_private = FALSE)))))) AND issue_relations.issue_from_id IN (800, 6373, 4842, 3201, 842, 4856);
-- Rewritten Queries
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_to_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13)) AND issues.is_private = False AND issue_relations.issue_from_id IN (3099, 2864, 243, 1944, 2426, 4492) LIMIT 1;
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_to_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking' LIMIT 1) IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13)) AND issues.is_private = False AND issue_relations.issue_from_id IN (243, 1944, 2426, 4492, 813, 3028) LIMIT 1;
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_to_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13) LIMIT 1) AND issues.is_private = False AND issue_relations.issue_from_id IN (243, 1944, 2426, 4492, 813, 3028) LIMIT 1;
SELECT issue_relations.* FROM issue_relations INNER JOIN issues ON issues.id = issue_relations.issue_to_id INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status <> 9 AND (SELECT 1 AS "one" FROM enabled_modules AS em WHERE em.project_id = projects.id AND em.name = 'issue_tracking' LIMIT 1) IS NOT NULL AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13) LIMIT 1) AND issues.is_private = False AND issue_relations.issue_from_id IN (2426, 4492, 813, 3028, 5114, 3013) LIMIT 1;
