...
-- Original Query
SELECT users.* FROM users INNER JOIN groups_users ON users.id = groups_users.group_id WHERE users.type IN ('Group', 'GroupBuiltin', 'GroupAnonymous', 'GroupNonMember') AND groups_users.user_id = 8 ORDER BY users.id ASC LIMIT 6;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('Group', 'GroupBuiltin', 'GroupAnonymous', 'GroupNonMember') ORDER BY users.id ASC LIMIT 6;
