...
-- Original Query
SELECT users.* FROM users INNER JOIN watchers ON users.id = watchers.user_id WHERE watchers.watchable_id = 9136 AND watchers.watchable_type = 'News' AND users.status = 2;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.status = 2 LIMIT 1;
