...
-- Original Query
SELECT  projects.* FROM projects WHERE projects.status = 9 AND (projects.id IN (SELECT em.project_id FROM enabled_modules em WHERE em.name='repository')) AND projects.identifier = 'qamswuxxanagpoowbh' LIMIT 3;
-- Rewritten Queries
SELECT projects.* FROM projects WHERE projects.status = 9 AND projects.id IN (SELECT em.project_id FROM enabled_modules AS em WHERE em.name = 'repository' LIMIT 1) AND projects.identifier = 'qamswuxxanagpoowbh' LIMIT 3;
