...
-- Original Query
SELECT members.* FROM members INNER JOIN projects ON projects.id = members.project_id WHERE members.user_id = 2936 AND projects.status <> 1 AND projects.status <> 9 AND projects.is_public = True AND projects.id NOT IN (SELECT project_id FROM members WHERE user_id IN (6, 13));
-- Rewritten Queries
SELECT members.* FROM members WHERE members.user_id = 2936;
SELECT members.* FROM members WHERE members.user_id = 2936 LIMIT 1;
