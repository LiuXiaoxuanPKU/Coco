...
-- Original Query
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 1 ORDER BY users.firstname, users.lastname, users.id;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 1 ORDER BY users.firstname, users.lastname, users.id LIMIT 1;
