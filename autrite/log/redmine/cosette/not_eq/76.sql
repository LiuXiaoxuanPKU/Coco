...
-- Original Query
SELECT trackers.* FROM trackers INNER JOIN projects_trackers ON trackers.id = projects_trackers.tracker_id WHERE projects_trackers.project_id = 2313 AND trackers.id = 554 ORDER BY trackers.position ASC LIMIT 9;
-- Rewritten Queries
SELECT trackers.* FROM trackers WHERE trackers.id = 554 ORDER BY trackers.position ASC LIMIT 9;
