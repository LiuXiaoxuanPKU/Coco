...
-- Original Query
SELECT 1 AS "one" FROM members INNER JOIN projects ON projects.id = members.project_id INNER JOIN member_roles ON member_roles.member_id = members.id WHERE members.user_id = 3627 AND projects.status <> 1 AND member_roles.role_id IN (2, 1) LIMIT 6;
-- Rewritten Queries
SELECT 1 AS "one" FROM members INNER JOIN member_roles ON member_roles.member_id = members.id WHERE members.user_id = 3627 AND member_roles.role_id IN (1, 2) LIMIT 6;
SELECT 1 AS "one" FROM members WHERE members.user_id = 3627 LIMIT 6;
