...
-- Original Query
SELECT COUNT(*) FROM projects WHERE projects.status = 1 AND (projects.id IN (SELECT em.project_id FROM enabled_modules em WHERE em.name='repository'));
-- Rewritten Queries
...
SELECT COUNT(*) FROM projects WHERE projects.status = 1 AND projects.id IN (SELECT em.project_id FROM enabled_modules AS em WHERE em.name = 'repository' LIMIT 1) LIMIT 1;
