...
-- Original Query
SELECT issues.* FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issues.project_id = 3166 AND issue_statuses.is_closed = True ORDER BY issues.id ASC LIMIT 2;
-- Rewritten Queries
SELECT issues.* FROM issues WHERE issues.project_id = 3166 ORDER BY issues.id ASC LIMIT 2;
