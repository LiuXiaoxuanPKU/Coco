...
-- Original Query
SELECT projects.* FROM projects WHERE projects.status = 9 AND projects.id IN (SELECT em.project_id FROM enabled_modules AS em WHERE em.name = 'repository') ORDER BY projects.identifier;
-- Rewritten Queries
SELECT projects.* FROM projects WHERE projects.status = 9 AND projects.id IN (SELECT em.project_id FROM enabled_modules AS em WHERE em.name = 'repository') ORDER BY projects.identifier LIMIT 1;
SELECT projects.* FROM projects WHERE projects.status = 9 AND projects.id IN (SELECT em.project_id FROM enabled_modules AS em WHERE em.name = 'repository' LIMIT 1) ORDER BY projects.identifier;
SELECT projects.* FROM projects WHERE projects.status = 9 AND projects.id IN (SELECT em.project_id FROM enabled_modules AS em WHERE em.name = 'repository' LIMIT 1) ORDER BY projects.identifier LIMIT 1;
