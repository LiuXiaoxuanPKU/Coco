...
-- Original Query
SELECT versions.* FROM versions INNER JOIN projects ON projects.id = versions.project_id WHERE (projects.lft >= 13 AND projects.rgt <= 18 AND projects.status <> 9);
-- Rewritten Queries
SELECT versions.* FROM versions;
SELECT versions.* FROM versions INNER JOIN projects ON projects.id = versions.project_id WHERE projects.lft >= 13 AND projects.rgt <= 18 AND projects.status <> 9 LIMIT 1;
