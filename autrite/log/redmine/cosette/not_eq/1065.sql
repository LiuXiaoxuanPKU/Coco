...
-- Original Query
SELECT issues.id FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE ((projects.lft >= 1 AND projects.rgt <= 10));
-- Rewritten Queries
SELECT issues.id FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE projects.lft >= 1 AND projects.rgt <= 10 LIMIT 1;
