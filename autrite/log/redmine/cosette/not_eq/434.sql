...
-- Original Query
SELECT DISTINCT users.* FROM users INNER JOIN members ON members.user_id = users.id INNER JOIN member_roles ON member_roles.member_id = members.id INNER JOIN roles ON roles.id = member_roles.role_id WHERE users.status = 1 AND users.type IN ('User', 'User') AND members.project_id = 4746 AND roles.assignable = True ORDER BY users.type DESC, users.firstname, users.lastname, users.id;
-- Rewritten Queries
