...
-- Original Query
SELECT users.id FROM users WHERE users.status = 1 AND (users.id = 89 OR users.id IN (SELECT user_id FROM members WHERE project_id IN (1,3,4,6)));
-- Rewritten Queries
SELECT users.id FROM users WHERE users.status = 1 AND (users.id = 89 OR users.id IN (SELECT user_id FROM members WHERE project_id IN (1, 3, 4, 6))) LIMIT 1;
SELECT users.id FROM users WHERE users.status = 1 AND (users.id = 89 OR users.id IN (SELECT user_id FROM members WHERE project_id IN (1, 3, 4, 6) LIMIT 1));
SELECT users.id FROM users WHERE users.status = 1 AND (users.id = 89 OR users.id IN (SELECT user_id FROM members WHERE project_id IN (1, 3, 4, 6) LIMIT 1)) LIMIT 1;
