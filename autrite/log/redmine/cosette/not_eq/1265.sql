...
-- Original Query
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 ORDER BY login;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 ORDER BY login LIMIT 1;
