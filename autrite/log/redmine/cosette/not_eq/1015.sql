...
-- Original Query
SELECT issues.* FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status = 9;
-- Rewritten Queries
SELECT issues.* FROM issues;
SELECT issues.* FROM issues INNER JOIN projects ON projects.id = issues.project_id WHERE projects.status = 9 LIMIT 1;
