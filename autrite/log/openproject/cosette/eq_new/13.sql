...
-- Original Query
SELECT 1 AS "one" FROM versions WHERE LOWER(versions.name) = 'knux' AND versions.project_id IS NULL LIMIT 2;
-- Rewritten Queries
SELECT 1 AS "one" FROM versions WHERE LOWER(versions.name) = 'knux' AND False LIMIT 2;
