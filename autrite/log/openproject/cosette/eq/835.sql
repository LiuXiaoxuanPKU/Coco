...
-- Original Query
SELECT DISTINCT users.id FROM users LEFT OUTER JOIN members ON users.id = members.user_id AND members.project_id = 1207 LEFT OUTER JOIN member_roles ON members.id = member_roles.member_id LEFT OUTER JOIN roles ON roles.id = member_roles.role_id WHERE users.type IN ('Group', 'User', 'User', 'User') AND (roles.id IS NOT NULL OR users.admin = True);
-- Rewritten Queries
SELECT users.id FROM users LEFT OUTER JOIN members ON users.id = members.user_id AND members.project_id = 1207 LEFT OUTER JOIN member_roles ON members.id = member_roles.member_id LEFT OUTER JOIN roles ON roles.id = member_roles.role_id WHERE users.type IN ('Group', 'Group', 'User', 'User') AND (roles.id IS NOT NULL OR users.admin = True);
