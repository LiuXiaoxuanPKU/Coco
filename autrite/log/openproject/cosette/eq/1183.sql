...
-- Original Query
SELECT DISTINCT users.* FROM users LEFT OUTER JOIN members ON users.id = members.user_id AND members.project_id = 18 LEFT OUTER JOIN member_roles ON members.id = member_roles.member_id LEFT OUTER JOIN roles ON roles.id = member_roles.role_id LEFT OUTER JOIN role_permissions ON roles.id = role_permissions.role_id AND role_permissions.permission = 'view_work_packages' WHERE users.type IN ('Group', 'User', 'Group') AND (roles.id IS NOT NULL AND role_permissions.id IS NOT NULL OR users.admin = True);
-- Rewritten Queries
SELECT users.* FROM users LEFT OUTER JOIN members ON users.id = members.user_id AND members.project_id = 18 LEFT OUTER JOIN member_roles ON members.id = member_roles.member_id LEFT OUTER JOIN roles ON roles.id = member_roles.role_id LEFT OUTER JOIN role_permissions ON roles.id = role_permissions.role_id AND role_permissions.permission = 'view_work_packages' WHERE users.type IN ('Group', 'Group', 'User') AND (roles.id IS NOT NULL AND role_permissions.id IS NOT NULL OR users.admin = True);
