...
-- Original Query
SELECT notification_settings.* FROM notification_settings WHERE notification_settings.involved = False AND notification_settings.user_id IN (SELECT users.id FROM users WHERE users.type IN ('User', 'Group') AND users.id IS NULL AND users.id IN (SELECT DISTINCT users.id FROM users LEFT OUTER JOIN members ON users.id = members.user_id AND members.project_id = 9 LEFT OUTER JOIN member_roles ON members.id = member_roles.member_id LEFT OUTER JOIN roles ON roles.id = member_roles.role_id LEFT OUTER JOIN role_permissions ON roles.id = role_permissions.role_id AND role_permissions.permission = 'view_work_packages' WHERE users.type IN ('Group', 'User') AND (roles.id IS NOT NULL AND role_permissions.id IS NOT NULL OR users.admin = True))) AND notification_settings.id IN (SELECT COALESCE(project_settings.id, notification_settings.id) FROM notification_settings LEFT OUTER JOIN notification_settings AS project_settings ON project_settings.project_id = 9 AND notification_settings.user_id = project_settings.user_id WHERE notification_settings.project_id IS NULL);
-- Rewritten Queries
SELECT notification_settings.* FROM notification_settings WHERE notification_settings.involved = False AND notification_settings.user_id IN (SELECT users.id FROM users WHERE users.type IN ('User', 'Group') AND users.id IS NULL AND users.id IN (SELECT DISTINCT users.id FROM users LEFT OUTER JOIN members ON users.id = members.user_id AND members.project_id = 9 LEFT OUTER JOIN member_roles ON members.id = member_roles.member_id LEFT OUTER JOIN roles ON roles.id = member_roles.role_id LEFT OUTER JOIN role_permissions ON roles.id = role_permissions.role_id AND role_permissions.permission = 'view_work_packages' WHERE users.type IN ('Group', 'User') AND (roles.id IS NOT NULL AND role_permissions.id IS NOT NULL OR users.admin = True))) AND notification_settings.id IN (SELECT COALESCE(project_settings.id, notification_settings.id) FROM notification_settings INNER JOIN notification_settings AS project_settings ON project_settings.project_id = 9 AND notification_settings.user_id = project_settings.user_id WHERE notification_settings.project_id IS NULL);
