...
-- Original Query
SELECT COUNT(DISTINCT notifications.id) FROM notifications LEFT OUTER JOIN projects ON projects.id = notifications.project_id LEFT OUTER JOIN users ON users.id = notifications.actor_id AND users.type IN ('Group', 'Group', 'Group', 'User') WHERE notifications.read_ian IS NOT NULL AND notifications.recipient_id IS NULL AND notifications.resource_type = 'Journal' AND notifications.resource_id IN (SELECT work_packages.id FROM work_packages WHERE work_packages.project_id IN (SELECT DISTINCT projects.id FROM projects INNER JOIN enabled_modules ON projects.id = enabled_modules.project_id AND enabled_modules.name IN ('work_package_tracking') AND projects.active = True WHERE projects.active = True));
-- Rewritten Queries
SELECT COUNT(DISTINCT notifications.id) FROM notifications LEFT OUTER JOIN projects ON projects.id = notifications.project_id LEFT OUTER JOIN users ON users.id = notifications.actor_id AND users.type IN ('Group', 'Group', 'Group', 'User') WHERE notifications.read_ian IS NOT NULL AND notifications.recipient_id IS NULL AND notifications.resource_type = 'Journal' AND notifications.resource_id IN (SELECT work_packages.id FROM work_packages WHERE work_packages.project_id IN (SELECT DISTINCT projects.id FROM projects INNER JOIN enabled_modules ON projects.id = enabled_modules.project_id AND enabled_modules.name IN ('work_package_tracking') AND projects.active = True WHERE projects.active = True)) LIMIT 1;
SELECT COUNT(DISTINCT notifications.id) FROM notifications LEFT OUTER JOIN projects ON projects.id = notifications.project_id LEFT OUTER JOIN users ON users.id = notifications.actor_id AND users.type IN ('Group', 'Group', 'Group', 'User') WHERE notifications.read_ian IS NOT NULL AND notifications.recipient_id IS NULL AND notifications.resource_type = 'Journal' AND notifications.resource_id IN (SELECT work_packages.id FROM work_packages WHERE work_packages.project_id IN (SELECT DISTINCT projects.id FROM projects INNER JOIN enabled_modules ON projects.id = enabled_modules.project_id AND enabled_modules.name IN ('work_package_tracking') AND projects.active = True WHERE projects.active = True LIMIT 1));
