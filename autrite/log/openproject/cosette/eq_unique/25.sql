...
-- Original Query
SELECT 1 AS "one" FROM menu_items WHERE menu_items.name IS NULL AND menu_items.navigatable_id IS NULL AND menu_items.parent_id IS NULL LIMIT 10;
-- Rewritten Queries
SELECT 1 AS "one" FROM menu_items WHERE False AND menu_items.navigatable_id IS NULL AND menu_items.parent_id IS NULL LIMIT 10;
