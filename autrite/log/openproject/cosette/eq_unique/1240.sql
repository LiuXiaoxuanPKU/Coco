...
-- Original Query
SELECT DISTINCT users.id FROM users LEFT OUTER JOIN members ON users.id = members.user_id AND members.project_id = 122 LEFT OUTER JOIN member_roles ON members.id = member_roles.member_id LEFT OUTER JOIN roles ON roles.id = member_roles.role_id LEFT OUTER JOIN role_permissions ON roles.id = role_permissions.role_id AND role_permissions.permission = 'view_work_packages' WHERE users.type IN ('User', 'Group', 'User', 'Group') AND (roles.id IS NOT NULL AND role_permissions.id IS NOT NULL OR users.admin = True);
-- Rewritten Queries
SELECT users.id FROM users LEFT OUTER JOIN members ON users.id = members.user_id AND members.project_id = 122 LEFT OUTER JOIN member_roles ON members.id = member_roles.member_id LEFT OUTER JOIN roles ON roles.id = member_roles.role_id LEFT OUTER JOIN role_permissions ON roles.id = role_permissions.role_id AND role_permissions.permission = 'view_work_packages' WHERE users.type IN ('User', 'User', 'Group', 'User') AND (roles.id IS NOT NULL AND role_permissions.id IS NOT NULL OR users.admin = True);
