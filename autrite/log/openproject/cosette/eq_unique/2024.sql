...
-- Original Query
SELECT derived_start_date, derived_due_date FROM (SELECT LEAST(MIN(descendants_work_packages.start_date), MIN(descendants_work_packages.due_date)) AS derived_start_date, GREATEST(MAX(descendants_work_packages.start_date), MAX(descendants_work_packages.due_date)) AS derived_due_date FROM work_packages LEFT OUTER JOIN relations ON relations.hierarchy <> 0 AND relations.relates = 0 AND relations.duplicates = 0 AND relations.follows = 0 AND relations.blocks = 0 AND relations.includes = 0 AND relations.requires = 0 AND relations.from_id = work_packages.id LEFT OUTER JOIN work_packages AS descendants_work_packages ON descendants_work_packages.id = relations.to_id WHERE work_packages.id = 1229 GROUP BY work_packages.id) AS subquery;
-- Rewritten Queries
SELECT derived_start_date, derived_due_date FROM (SELECT LEAST(MIN(descendants_work_packages.start_date), MIN(descendants_work_packages.due_date)) AS derived_start_date, GREATEST(MAX(descendants_work_packages.start_date), MAX(descendants_work_packages.due_date)) AS derived_due_date FROM work_packages LEFT OUTER JOIN relations ON relations.hierarchy <> 0 AND relations.relates = 0 AND relations.duplicates = 0 AND relations.follows = 0 AND relations.blocks = 0 AND relations.includes = 0 AND relations.requires = 0 AND relations.from_id = work_packages.id LEFT OUTER JOIN work_packages AS descendants_work_packages ON descendants_work_packages.id = relations.to_id WHERE work_packages.id = 1229 GROUP BY work_packages.id) AS subquery LIMIT 1;
