...
def test_redmine_enumerate():
    ...
    q_before_str = "SELECT issues.id AS t0_r0, issues.tracker_id AS t0_r1, issues.project_id AS t0_r2, \
                    ...
                    issues.parent_id AS t0_r18, issues.root_id AS t0_r19, issues.lft AS t0_r20, \
                    issues.rgt AS t0_r21, issues.is_private AS t0_r22, issues.closed_on AS t0_r23, \
                    issue_statuses.id AS t1_r0, issue_statuses.name AS t1_r1, issue_statuses.is_closed AS t1_r2, \
                    issue_statuses.position AS t1_r3, issue_statuses.default_done_ratio AS t1_r4, \
                    projects.id AS t2_r0, projects.name AS t2_r1, projects.description AS t2_r2, \
                    projects.homepage AS t2_r3, projects.is_public AS t2_r4, projects.parent_id AS t2_r5, \
                    projects.created_on AS t2_r6, projects.updated_on AS t2_r7, projects.identifier AS t2_r8, \
                    projects.status AS t2_r9, projects.lft AS t2_r10, projects.rgt AS t2_r11, projects.inherit_members AS t2_r12, \
                    projects.default_version_id AS t2_r13, projects.default_assigned_to_id AS t2_r14 FROM issues \
                    INNER JOIN projects ON projects.id = issues.project_id \
                    INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id \
                    WHERE projects.status <> 9 AND \
                        (SELECT 1 AS one FROM enabled_modules AS em \
                            WHERE em.project_id = projects.id AND em.name = 'issue_tracking') IS NOT NULL \
                            AND projects.is_public = True AND \
                                projects.id NOT IN \
                                    (SELECT project_id FROM members WHERE user_id IN (6, 13)) \
                                    AND issues.is_private = False AND \
                                    issues.status_id IN \
                                        (SELECT id FROM issue_statuses WHERE is_closed = False) \
                                    AND issues.created_on <= '2021-08-23 23:59:59.999999' \
                    ORDER BY issues.id DESC LIMIT '$1' OFFSET '$2'"
    test_rewrite_helper(constraints, q_before_str)
