...
-- Original Query
SELECT members.* FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 2 AND users.status = 1;
-- Rewritten Queries
SELECT members.* FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 2 AND users.status = 1 LIMIT 1;
