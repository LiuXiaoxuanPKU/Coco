...
-- Original Query
SELECT issues.id FROM issues WHERE issues.root_id = 13 AND issues.lft > 1 AND issues.rgt < 6 ORDER BY issues.lft ASC;
-- Rewritten Queries
SELECT issues.id FROM issues WHERE issues.root_id = 13 AND issues.lft > 1 AND issues.rgt < 6 ORDER BY issues.lft ASC LIMIT 1;
