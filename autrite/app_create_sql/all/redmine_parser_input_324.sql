...
-- Original Query
SELECT users.* FROM users WHERE users.status = 2 AND users.status = 2 AND users.type IN ('User', 'User') AND (LOWER(users.login) LIKE LOWER('%watch%') OR users.id IN (SELECT user_id FROM email_addresses WHERE LOWER(address) LIKE LOWER('%watch%')) OR LOWER(users.firstname) LIKE LOWER('%watch%') OR LOWER(users.lastname) LIKE LOWER('%watch%')) ORDER BY users.type DESC, users.firstname, users.lastname, users.id LIMIT 9;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.status = 2 AND users.status = 2 AND users.type IN ('User', 'User') AND (LOWER(users.login) LIKE LOWER('%watch%') OR users.id IN (SELECT user_id FROM email_addresses WHERE LOWER(address) LIKE LOWER('%watch%') LIMIT 1) OR LOWER(users.firstname) LIKE LOWER('%watch%') OR LOWER(users.lastname) LIKE LOWER('%watch%')) ORDER BY users.type DESC, users.firstname, users.lastname, users.id LIMIT 9;
