...
-- Original Query
SELECT SUM(time_entries.hours) FROM issues INNER JOIN time_entries ON time_entries.issue_id = issues.id WHERE issues.root_id = 5 AND issues.lft >= 1 AND issues.rgt <= 10;
-- Rewritten Queries
SELECT SUM(time_entries.hours) FROM issues INNER JOIN time_entries ON time_entries.issue_id = issues.id WHERE issues.root_id = 5 AND issues.lft >= 1 AND issues.rgt <= 10 LIMIT 1;
