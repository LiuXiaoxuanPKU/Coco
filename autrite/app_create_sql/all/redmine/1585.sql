...
-- Original Query
SELECT issues.id FROM issues WHERE issues.root_id = 9 AND issues.lft > 2 AND issues.rgt < 5 ORDER BY issues.lft ASC;
-- Rewritten Queries
SELECT issues.id FROM issues WHERE issues.root_id = 9 AND issues.lft > 2 AND issues.rgt < 5 ORDER BY issues.lft ASC LIMIT 1;
