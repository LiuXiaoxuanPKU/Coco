...
-- Original Query
SELECT issues.id FROM issues WHERE issues.root_id = 8 AND issues.lft > 1 AND issues.rgt < 4 ORDER BY issues.lft ASC;
-- Rewritten Queries
SELECT issues.id FROM issues WHERE issues.root_id = 8 AND issues.lft > 1 AND issues.rgt < 4 ORDER BY issues.lft ASC LIMIT 1;
