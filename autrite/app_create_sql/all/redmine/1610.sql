...
-- Original Query
SELECT members.* FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 5 AND users.type IN ('User', 'User');
-- Rewritten Queries
SELECT members.* FROM members INNER JOIN users ON users.id = members.user_id WHERE members.project_id = 5 AND users.type IN ('User', 'User') LIMIT 1;
