...
-- Original Query
SELECT SUM(CAST(custom_values.value AS DECIMAL(30, 3))) FROM projects INNER JOIN custom_values ON custom_values.customized_id = projects.id AND custom_values.customized_type = 'Principal' WHERE projects.status <> 9 AND projects.status IN ('1') AND custom_values.custom_field_id = 5 AND custom_values.value <> '';
-- Rewritten Queries
SELECT SUM(CAST(custom_values.value AS DECIMAL(30, 3))) FROM projects INNER JOIN custom_values ON custom_values.customized_id = projects.id AND custom_values.customized_type = 'Principal' WHERE projects.status <> 9 AND projects.status IN ('1') AND custom_values.custom_field_id = 5 AND custom_values.value <> '' LIMIT 1;
