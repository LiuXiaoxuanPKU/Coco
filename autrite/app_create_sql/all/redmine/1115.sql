...
-- Original Query
SELECT SUM(time_entries.hours) FROM issues INNER JOIN time_entries ON time_entries.issue_id = issues.id WHERE issues.root_id = 14 AND issues.lft >= 1 AND issues.rgt <= 14;
-- Rewritten Queries
SELECT SUM(time_entries.hours) FROM issues INNER JOIN time_entries ON time_entries.issue_id = issues.id WHERE issues.root_id = 14 AND issues.lft >= 1 AND issues.rgt <= 14 LIMIT 1;
