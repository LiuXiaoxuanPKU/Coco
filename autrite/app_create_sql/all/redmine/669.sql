...
-- Original Query
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals INNER JOIN issues ON issues.id = journals.journalized_id INNER JOIN projects ON projects.id = issues.project_id WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (2251, 6555, 3612, 3929, 2395, 5606, 6950, 2687, 550, 6959, 9714) AND (journals.private_notes = False OR journals.user_id = 2 OR 1 = 0) AND journals.notes <> '{}' GROUP BY journals.journalized_id;
-- Rewritten Queries
