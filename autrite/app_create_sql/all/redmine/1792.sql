...
-- Original Query
SELECT users.* FROM users WHERE users.status = 2 AND users.status = 2 AND users.type IN ('User', 'User') AND users.id IN (1487, 1182);
-- Rewritten Queries
SELECT users.* FROM users WHERE users.status = 2 AND users.status = 2 AND users.type IN ('User', 'User') AND users.id IN (1487, 1182) LIMIT 1;
