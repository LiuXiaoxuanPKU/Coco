...
-- Original Query
SELECT SUM(COALESCE(estimated_hours, 20.0) * 100) FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issues.fixed_version_id = 2 AND issue_statuses.is_closed = True;
-- Rewritten Queries
SELECT SUM(COALESCE(estimated_hours, 20.0) * 100) FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issues.fixed_version_id = 2 AND issue_statuses.is_closed = True LIMIT 1;
