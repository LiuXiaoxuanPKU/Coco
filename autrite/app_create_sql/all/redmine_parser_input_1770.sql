...
-- Original Query
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.login = 'nlcchdduoymxgpfhsggbthmyytvqaasnxmtmsxxbwatyzzazmyxoeomj';
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.login = 'nlcchdduoymxgpfhsggbthmyytvqaasnxmtmsxxbwatyzzazmyxoeomj' LIMIT 1;
