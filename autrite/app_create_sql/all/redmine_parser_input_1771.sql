...
-- Original Query
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND users.login = 'bslipirgummsbwiiyvsbqrwyqkxzrtlunhdkwwnxfhxvaormwdy';
-- Rewritten Queries
SELECT users.* FROM users WHERE users.type IN ('User', 'AnonymousUser') AND users.status = 2 AND users.login = 'bslipirgummsbwiiyvsbqrwyqkxzrtlunhdkwwnxfhxvaormwdy' LIMIT 1;
