...
-- Original Query
SELECT SUM(COALESCE(estimated_hours, 23.75) * done_ratio) FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issues.fixed_version_id = 2 AND issue_statuses.is_closed = False;
-- Rewritten Queries
SELECT SUM(COALESCE(estimated_hours, 23.75) * done_ratio) FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issues.fixed_version_id = 2 AND issue_statuses.is_closed = False LIMIT 1;
