...
-- Original Query
SELECT 1 AS one FROM users WHERE LOWER(users.login) LIKE LOWER('%somenet%') OR users.id IN (SELECT user_id FROM email_addresses WHERE LOWER(address) LIKE LOWER('%somenet%')) OR LOWER(users.firstname) LIKE LOWER('%somenet%') OR LOWER(users.lastname) LIKE LOWER('%somenet%') LIMIT 8;
-- Rewritten Queries
SELECT 1 AS one FROM users WHERE LOWER(users.login) LIKE LOWER('%somenet%') OR users.id IN (SELECT user_id FROM email_addresses WHERE LOWER(address) LIKE LOWER('%somenet%') LIMIT 1) OR LOWER(users.firstname) LIKE LOWER('%somenet%') OR LOWER(users.lastname) LIKE LOWER('%somenet%') LIMIT 8;
