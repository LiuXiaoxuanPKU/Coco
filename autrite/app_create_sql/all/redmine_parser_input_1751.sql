...
-- Original Query
SELECT users.* FROM users WHERE users.status = 1 AND users.status = 1 AND users.type IN ('User', 'User') AND users.id = 263;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.status = 1 AND users.status = 1 AND users.type IN ('User', 'User') AND users.id = 263 LIMIT 1;
