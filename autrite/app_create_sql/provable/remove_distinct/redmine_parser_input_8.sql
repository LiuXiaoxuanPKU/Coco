...
-- Rewritten Queries
SELECT users.* FROM users INNER JOIN email_addresses ON email_addresses.user_id = users.id WHERE users.type IN ('$1', '$2') AND users.status = '$3' AND LOWER(email_addresses.address) IN ('testuser@example.org');
