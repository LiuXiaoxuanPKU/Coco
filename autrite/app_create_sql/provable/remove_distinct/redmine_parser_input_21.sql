...
-- Rewritten Queries
SELECT users.* FROM users INNER JOIN members ON members.user_id = users.id WHERE users.type IN ('$1', '$2') AND users.status = '$3' AND members.project_id = 8 ORDER BY users.firstname, users.lastname, users.id;
