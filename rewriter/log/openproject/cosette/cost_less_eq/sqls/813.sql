...
-- Original Query
SELECT user_id FROM roles INNER JOIN member_roles ON member_roles.role_id = roles.id INNER JOIN members ON members.id = member_roles.member_id LEFT OUTER JOIN role_permissions ON role_permissions.role_id = roles.id WHERE roles.builtin = 0 AND roles.type = 'GlobalRole' AND roles.id = 7341 ORDER BY "position";
-- Rewritten Queries
SELECT user_id FROM roles INNER JOIN member_roles ON member_roles.role_id = roles.id INNER JOIN members ON members.id = member_roles.member_id WHERE roles.builtin = 0 AND roles.type = 'GlobalRole' AND roles.id = 7341 ORDER BY "position";
