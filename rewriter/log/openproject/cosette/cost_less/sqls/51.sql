...
-- Original Query
SELECT COUNT(*) FROM changes INNER JOIN changesets ON changesets.id = changes.changeset_id WHERE changes.changeset_id IN (SELECT changesets.id FROM changesets WHERE changesets.repository_id = 9128 ORDER BY changesets.committed_on DESC, changesets.id DESC);
-- Rewritten Queries
SELECT COUNT(*) FROM changes WHERE changes.changeset_id IN (SELECT changesets.id FROM changesets WHERE changesets.repository_id = 9128 ORDER BY changesets.committed_on DESC, changesets.id DESC);
