...
-- Original Query
SELECT trackers.* FROM trackers INNER JOIN projects_trackers ON trackers.id = projects_trackers.tracker_id WHERE projects_trackers.project_id = 7809 AND trackers.id IN (3368, 9392) ORDER BY trackers.position ASC;
-- Rewritten Queries
SELECT trackers.* FROM trackers WHERE trackers.id IN (4306, 2540) ORDER BY trackers.position ASC;
