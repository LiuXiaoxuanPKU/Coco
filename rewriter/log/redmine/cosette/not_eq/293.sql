...
-- Original Query
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals INNER JOIN issues ON issues.id = journals.journalized_id INNER JOIN projects ON projects.id = issues.project_id WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (352, 3142, 5556) AND (journals.private_notes = False OR journals.user_id = 2 OR projects.status <> 9 AND projects.id IN (1, 5)) GROUP BY journals.journalized_id;
-- Rewritten Queries
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals INNER JOIN issues ON issues.id = journals.journalized_id WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (5264, 21, 3441) AND (journals.private_notes = False OR journals.user_id = 2) GROUP BY journals.journalized_id;
SELECT MAX(journals.id) AS maximum_id, journals.journalized_id AS journals_journalized_id FROM journals WHERE journals.journalized_type = 'Issue' AND journals.journalized_id IN (1412, 4506, 7411) AND (journals.private_notes = False OR journals.user_id = 2) GROUP BY journals.journalized_id;
