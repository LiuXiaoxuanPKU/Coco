...
-- Original Query
SELECT trackers.id FROM trackers INNER JOIN projects_trackers ON trackers.id = projects_trackers.tracker_id WHERE projects_trackers.project_id = 9615 AND trackers.id IN (1239, 4397) ORDER BY trackers.position ASC;
-- Rewritten Queries
SELECT trackers.id FROM trackers WHERE trackers.id IN (9868, 3276) ORDER BY trackers.position ASC;
