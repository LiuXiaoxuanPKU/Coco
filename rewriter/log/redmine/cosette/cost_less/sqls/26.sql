...
-- Original Query
SELECT users.* FROM users WHERE users.status IN (1, 2) AND users.id IN (SELECT DISTINCT user_id FROM members WHERE project_id IN (1, 3, 4, 6)) AND users.status = 2;
-- Rewritten Queries
SELECT users.* FROM users WHERE users.status IN (1, 2) AND users.id IN (SELECT user_id FROM members WHERE project_id IN (1, 3, 4, 6)) AND users.status = 2;
