...
-- Original Query
SELECT DISTINCT workflows.old_status_id, workflows.new_status_id FROM workflows WHERE workflows.type IN ('WorkflowTransition') AND workflows.tracker_id IN (11521, 18461, 15413);
-- Rewritten Queries
SELECT workflows.old_status_id, workflows.new_status_id FROM workflows WHERE workflows.type IN ('WorkflowTransition') AND workflows.tracker_id IN (10813, 11521, 18461);
