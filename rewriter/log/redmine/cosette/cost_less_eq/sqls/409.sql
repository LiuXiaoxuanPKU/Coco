...
-- Rewritten Queries
SELECT queries.* FROM queries WHERE queries.type IN ('ProjectQuery') AND queries.project_id IS NULL AND (queries.visibility <> 0 OR queries.user_id = 1) AND queries.project_id IS NULL ORDER BY queries.name ASC, queries.id ASC;
