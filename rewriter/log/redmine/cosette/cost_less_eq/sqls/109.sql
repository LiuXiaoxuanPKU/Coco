...
-- Original Query
SELECT 1 AS "one" FROM issues INNER JOIN issue_statuses ON issue_statuses.id = issues.status_id WHERE issues.root_id = 8278 AND issues.lft > 1 AND issues.rgt < 10 AND issue_statuses.is_closed = True LIMIT 2;
-- Rewritten Queries
SELECT 1 AS "one" FROM issues WHERE issues.root_id = 8278 AND issues.lft > 1 AND issues.rgt < 10 LIMIT 2;
